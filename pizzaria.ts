// Importações de bibliotecas do Node.js
import * as path from 'path'; // Manipulação de caminhos de arquivos e pastas
import { promises as fs } from 'fs'; // Manipulação de arquivos assíncrona (promises)
import * as readline from 'readline'; // Para interagir com o usuário no terminal
import { stdin as input, stdout as output } from 'process'; // Entrada e saída padrão do console

/* ---------------------- Tipos ---------------------- */
// Tipos específicos para organizar os dados da pizzaria
type Tamanho = "Pequena" | "Média" | "Grande";
type FormaPagamento = "Dinheiro" | "Cartão" | "PIX";

// Estrutura de um pedido (entrada de dados da pizzaria)
interface PizzariaEntrada {
  idPedido: number;              // ID único do pedido
  pedidoPizza: string;           // Nome da pizza escolhida
  pedidoBebida: string;          // Nome da bebida escolhida
  tamanhoPizza: Tamanho;         // Tamanho da pizza
  quantidadePizza: number;       // Quantidade de pizzas
  bordaRecheada: boolean;        // Se tem borda recheada ou não
  quantidadeBebidas: number;     // Quantidade de bebidas
  sobremesa: string;             // Sobremesa escolhida
  quantidadeSobremesa: number;   // Quantidade de sobremesas
  enderecoEntrega: string;       // Endereço do cliente
  cliente: string;               // Nome do cliente
  telefone: string;              // Telefone do cliente
  horaPedido: string;            // Hora do pedido (ISO string)
  formaPagamento?: FormaPagamento; // Forma de pagamento escolhida
}
...
// Arquivos que serão usados (CSV e JSON)

const ARQ = {
  entradas: path.join(DIR.csv, 'entradas.csv'),   // Registro de entradas
  ativos: path.join(DIR.csv, 'ativos.csv'),       // Pedidos em aberto
  historico: path.join(DIR.csv, 'historico.csv'), // Histórico de pedidos finalizados
  config: path.join(DIR.json, 'config.json'),     // Configurações de preços
  lastId: path.join(DIR.json, 'last_id.json'),    // Último ID usado
  sabores: path.join(DIR.json, 'sabores.json'),   // Lista de sabores
  sobremesas: path.join(DIR.json, 'sobremesas.json'), // Lista de sobremesas
};
...
// Criação dos arquivos CSV com cabeçalho
...
// Criação de arquivos JSON com dados padrão
...
// Converte objeto pedido em uma linha CSV
...
/* --------------- Helpers: ID e Config -------------------- */
// Função que gera próximo ID de pedido
...
/* --------------- Repositório CSV -------------------- */
// Funções para ler/escrever pedidos em arquivos CSV
...
async function main():Promise<void>{
  ...
  let sair=false;
  while(!sair){
    //MENU PRINCIPAL QUE VAI APARECER NO TERMINAL
    console.log('\n=== MENU ===');
